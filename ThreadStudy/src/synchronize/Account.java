...
/**
 * 일상생활의 은행계좌를 표현하기 위한 클래스(추상화) 객체에 대한 데이터타입 정의
 */
...
//		return  "입출금계좌\t"+ accountNum + "\t" + accountOwner + "\t*****\t" + restMoney;
		return String.format("%-9s", "입출금계좌") + String.format("%-17s", accountNum) + String.format("%-6s", accountOwner)
				+ String.format("%-7s", "*****") + String.format("%,14d원", restMoney);
	}
